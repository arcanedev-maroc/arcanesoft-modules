...
@section('page-title')
    <i class="fa fa-fw fa-user-secret"></i> @lang('Administrator\'s details')
@endsection
                ...
                <table class="table table-md mb-0">
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td class="text-right"><small class="text-muted">{{ $admin->last_activity }}</small></td>
                        </tr>
                        <tr>
                            <th>@lang('Created at') :</th>
                            <td class="text-right"><small class="text-muted">{{ $admin->created_at }}</small></td>
                        </tr>
                        <tr>
                            <th>@lang('Updated at') :</th>
                            <td class="text-right"><small class="text-muted">{{ $admin->updated_at }}</small></td>
                        </tr>
                        @if ($admin->trashed())
                            <tr>
                                <th>@lang('Deleted at') :</th>
                                <td class="text-right"><small class="text-muted">{{ $admin->deleted_at }}</small></td>
                            </tr>
                        @endif
                    </tbody>
                </table>
                ...
                {{ form()->open(['route' => ['admin::auth.administrators.activate', $admin], 'method' => 'PUT', 'id' => 'activate-user-form']) }}
                    <div class="modal-content">
                        ...
                        <div id="activateUserMessage" class="modal-body">
                            @lang($admin->isActive() ? 'Are you sure you want to deactivate user ?' : 'Are you sure you want to activate user ?')
                        </div>
                        <div class="modal-footer justify-content-between">
                            {{ arcanesoft\ui\action_button('cancel')->attribute('data-dismiss', 'modal') }}
                            {{ arcanesoft\ui\action_button($admin->isActive() ? 'deactivate' : 'activate')->submit() }}
                        </div>
                    </div>
                {{ form()->close() }}
            </div>
