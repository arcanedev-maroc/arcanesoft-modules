...
<?php $total = Illuminate\Support\Arr::pull($percents, 'all'); ?>
...
@section('content')
    <div class="card card-borderless card-log-level flex-row shadow-sm mb-3">
        ...
        <div class="card-body p-2">
            ...
            <h4>{{ $total['value'] }} <small>@lang('entries') <small>({{ $total['percent'] }} %)</small></small></h4>

            <div class="progress progress-xs progress-flat">
                <div class="progress-bar {{ $total['value'] > 0 ? "bg-log-level-all" : 'bg-log-level-empty' }}"
                     style="width: {{ $total['percent'] }}%;" aria-valuenow="{{ $total['percent'] }}"
                ...
                <div class="col-sm-6 col-xl-4">
                    <div class="card card-borderless card-log-level flex-row shadow-sm mb-3">
                        ...
                        <div class="card-body p-2">
                            ...
                            <h4>
                                {{ $log['value'] }} <small>@lang('entries')</small>
                            </h4>

                            <div class="progress progress-xs progress-flat">
                                <div class="progress-bar {{ $log['value'] > 0 ? "bg-log-level-{$level}" : 'bg-log-level-empty' }}"
                                     style="width: {{ $log['percent'] }}%;" aria-valuenow="{{ $log['percent'] }}"
                                     role="progressbar" aria-valuemin="0" aria-valuemax="100"></div>
                            </div>
                        </div>
                    </div>
