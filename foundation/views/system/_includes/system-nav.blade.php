    ...
    <div class="card-body p-2">
        <nav class="nav nav-pills nav-justified">
            @can(Arcanesoft\Foundation\System\Policies\InformationPolicy::ability('index'))
                <a href="{{ route('admin::system.index') }}" class="nav-item nav-link {{ active(['admin::system.index']) }}">
                    @lang('Information')
                </a>
            @endcan

            @can(Arcanesoft\Foundation\System\Policies\MaintenancePolicy::ability('index'))
                <a href="{{ route('admin::system.maintenance.index') }}" class="nav-item nav-link {{ active(['admin::system.maintenance.*']) }}">
                    @lang('Maintenance')
                </a>
            @endcan
