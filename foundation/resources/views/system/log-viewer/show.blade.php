...
@section('content')
    ...
    <div class="card card-borderless shadow-sm mb-3">
        ...
        <div class="card-footer p-2 text-right">
            ...
            {{ ui\action_button('delete')->attribute('onclick', "window.Foundation.\$emit('foundation::system.log-viewer.delete')")->size('sm') }}
            @endcan
        </div>
    </div>
    ...
    <section class="timeline-container">
        @foreach($entries as $key => $entry)
            ...
            <div class="timeline-item card card-borderless shadow-sm mb-3">
                <div class="timeline-dot shadow-sm text-white bg-log-level-{{ $entry->level }}">
                    {!! $entry->icon() !!}
                </div>
                <div class="card-header d-flex px-3 py-2">
                    ...
                    @if ($entry->hasStack())
                        <a class="btn btn-sm btn-secondary ml-auto" role="button" data-toggle="collapse" href="#log-entry-stack-{{ $key }}" aria-expanded="false" aria-controls="log-stack-{{ $key }}">
